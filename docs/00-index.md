...
当前 01 使用 `style_application_scope` 判断范围：凡是“做成 / 生成 / 套用 XX 风格”的请求，默认进入 `global_page_style`，至少包含背景、程序化面板、1 个固定角部面板饰品和页面配色规则。只有用户明确说“只 / 仅 / 单独 / 其他不变”时，才进入 `element_style_patch` 并限制到指定元素。
