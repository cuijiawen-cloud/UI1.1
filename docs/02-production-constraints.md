...
`icon_new: 0` 表示当前不允许新增任何独立图标资源；风格语义只能由背景图、程序化面板、固定局部面板装饰、UI primitive 或已有图标库承载。
...
4. 面板装饰最多使用 1 张固定局部 PNG，且不参与拉伸。
5. 不生成新图标；装饰语义优先由背景图、程序化面板、固定局部面板装饰、UI primitive 或已有图标库承载。
...
1. 当前组件超出默认资源的安全使用边界。
2. 已尝试调整程序化绘制参数、padding、opacity、tint、shadow、primitive 组合。
...
- 不是默认面板主体方案，只能作为极简、规则、低纹理面板 fallback。
- 必须是单张透明 PNG 源图。
...
| 固定局部装饰随面板比例缩放，或跨进可缩放区域后被拉伸 | 面板装饰贴片错误（blocker） | `02-production-constraints.md` |
