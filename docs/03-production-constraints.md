...
- 在显式局部请求中，因为“视觉更统一”“背景不协调”“整体更好看”等主观理由修改未授权表面，例如根背景、页面底色、预览页背景、页面级配色或非目标组件。
...
在面板皮肤范围内，AI / PNG 只允许负责不参与拉伸的角部贴片：

- 四角小装饰。
- 右下角或指定角落折角。
- 角部抽象材质片、线条或轮廓装饰。

该限制只约束面板系统，不限制页面背景。整体换肤时，`background_tool.png` 仍是必须生成或引用的页面背景资产，不能被程序化面板、纯色、渐变、NanoVG 纹理线条或局部光晕替代。
...
- 服务 UI，不做完整插画主视觉。
- 默认固定生成 1 张 `1920x1080` 背景图，作为应用 UI 的业务基准背景。
- 不再根据当前目标背景界面、背景容器、设计稿节点、工程 viewport、运行时测量或截图区域自动匹配背景尺寸。
- `1920x1080` 是明确的生产默认规格，不需要 current viewport evidence；不得因为页面是手机、电脑、平板或某个运行时尺寸而自动改成其他宽高。
- 多断点、多设备、横竖屏分别出图必须作为明确需求单独声明，不得无理由批量生成多尺寸背景。
- 运行时允许使用 `cover` 或居中裁切适配不同界面，但不允许拉伸变形；裁切适配是该固定规格策略的一部分，不再作为尺寸来源错误处理。
...
- 如果生成 prompt 只描述游戏风格、场景、氛围或美术关键词，而没有写入“服务 UI、不做完整插画主视觉、中央 45%-60% 干净低密度、装饰放边缘和角落、不得影响正文/表单/列表/按钮识别”，必须阻断生成。
- 如果背景输出规格不是固定 `1920x1080`，且没有明确的变更需求说明，必须阻断生成。
- 如果没有先读取并记录本文件的背景图硬约束，必须阻断生成。
- 如果没有在生成后用本文件约束做合规验收，不得把背景图引用进代码或物料配置。
- “XX 游戏风格古城 / 场景 / 插画 / 氛围图”这类纯风格 prompt 默认会导向完整插画级背景，不满足工具 UI 背景约束，必须改写为低密度 UI 支持背景 prompt 后才能生成。
...
| 背景生成前仍要求采集当前界面或背景容器宽高 | 旧尺寸采集逻辑未移除 | `01-workflow-dispatch.md` / `05-material-generation.md` |
| 背景输出不是固定 `1920x1080`，且没有明确变更需求 | background_rule 执行失败 | `05-material-generation.md` |
| 因设备类型、9:16、运行时 viewport 或截图区域自动改写背景尺寸 | 固定规格策略被绕过（blocker） | `01-workflow-dispatch.md` / `05-material-generation.md` |
...
| 用户明确“仅 / 只”改面板，但根背景、页面底色、预览页背景或页面级配色被修改 | scope violation（blocker） | `05-material-generation.md` |
