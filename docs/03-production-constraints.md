...
- 不得把“移动端竖屏”“手游工具”“9:16”或 `1080x1920`、`720x1280`、`1440x2560` 等常见手机预设当作当前界面尺寸来源。除非这些数值来自用户、设计稿、当前背景节点、工程 viewport、运行时测量或截图区域，否则一律视为 `missing_target_background_dimensions`。
- 默认只生成当前目标界面所需的 1 张背景图；多断点、多设备或横竖屏同时适配必须作为明确需求单独声明，不得无理由批量生成多尺寸背景。
- 不允许先生成一个任意比例背景，再依赖后续拉伸、裁切或居中缩放来冒充尺寸匹配。
- `cover`、`contain`、居中裁切、运行时缩放等只能作为显示层兜底，不能作为生成尺寸来源合规的理由。若生成尺寸不是当前目标背景界面的实际尺寸，即使运行时 `cover` 显示正常，也仍判定为尺寸来源不合规。
...
- 如果没有先取得当前目标背景界面或背景容器的具体宽高和 evidence，必须阻断生成。
...
| 用设备类型、9:16 或通用手机预设推导背景尺寸，或用 `cover` 解释尺寸不匹配 | 尺寸来源不合规（blocker） | `01-workflow-dispatch.md` / `05-material-generation.md` |
